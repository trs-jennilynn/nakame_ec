 ...
 * @version $Id: SC_Helper_Paypal.php 1165 2012-06-18 09:44:40Z adachi $
 */
class SC_Helper_Paypal {
    ...
    function setConfig($arrConfig) {
        ...
        $objQuery->update("dtb_module", array('sub_data' => serialize($arrConfig)),
                          "module_code = ?", array(PLG_PAYPAL_EXPRESS_CODE));
    }
    ...
    function getConfig() {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $config = $objQuery->get("sub_data", "dtb_module", "module_code = ?", array(PLG_PAYPAL_EXPRESS_CODE));
    ...
    function getPluginDir() {
        $plugin_dir = '';
        $plugin_dir = DATA_REALDIR . 'downloads/plugin/' . PLG_PAYPAL_EXPRESS_CODE . '/';
        return $plugin_dir;
    }
    ...
    function registerPayments() {
        $arrData = array();
        $arrData['payment_method'] = t('pp16');
        $arrData['module_path'] = PLUGIN_UPLOAD_REALDIR . PLG_PAYPAL_EXPRESS_CODE . "/paypal_link.php";
        $arrData['charge_flg'] = "1";
        $arrData['fix'] = 3;
        $arrData['creator_id'] = $_SESSION['member_id'];
        $arrData['create_date'] = "now()";
        $arrData['update_date'] = "now()";
        $arrData['memo03'] = PLG_PAYPAL_EXPRESS_CODE; // 決済モジュールを認識するためのデータ
        $arrData['del_flg'] = "0";
        ...
        // 支払方法データが存在すればUPDATE
        if ($exists > 0) {
            $objQuery->update("dtb_payment", $arrData, "memo03 = ?", array(PLG_PAYPAL_EXPRESS_CODE));
        }
        // 支払方法データが無ければINSERT
        else {
            // ランクの最大値を取得
...
__EOS__;
        ...
        return $objQuery->select($col, $from, $where, array(PLG_PAYPAL_EXPRESS_CODE, $product_type_id));
    }
    ...
    function getConfirmTplPath() {
        ...
        case DEVICE_TYPE_SMARTPHONE:
                return PLUGIN_UPLOAD_REALDIR . PLG_PAYPAL_EXPRESS_CODE . '/confirm_sphone_html5.tpl';
            break;

        case DEVICE_TYPE_PC:
        default:
        }
        return PLUGIN_UPLOAD_REALDIR . PLG_PAYPAL_EXPRESS_CODE . '/confirm.tpl';
    }
    ...
    function getErrorTplPath() {
        switch (SC_Display_Ex::detectDevice()) {
        case DEVICE_TYPE_SMARTPHONE:
            return PLUGIN_UPLOAD_REALDIR . PLG_PAYPAL_EXPRESS_CODE . '/error_sphone.tpl';
            break;

        case DEVICE_TYPE_PC:
        default:
        }
        return PLUGIN_UPLOAD_REALDIR . PLG_PAYPAL_EXPRESS_CODE . '/error.tpl';
    }
